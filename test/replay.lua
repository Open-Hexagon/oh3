...
describe("Replay files", function()
    it("can be created", function()
        local rp = Replay:new()
        rp:set_game_data(21, { invincible = true }, true, "player", "somepack", "somelevel", { somesetting = 10 })
        rp:record_seed(2323)
    ...
    it("can be loaded", function()
        ...
        assert.is.equal(rp.player_name, "player")
    ...
    it("can save and load inputs", function()
        local rp = Replay:new()
        rp:set_game_data(192, { debug = true }, false, "player", "somepack", "somelevel", { somesetting = false })
        local to_record = {
            [10] = {
                left = true,
            },
        ...
        assert.is.equal(loaded_rp.player_name, "player")
