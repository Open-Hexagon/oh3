...
describe("Replay files", function()
    it("can be created", function()
        local rp = Replay:new()
        rp:set_game_data({invincible = true}, 2323, "somepack", "somelevel", {somesetting = 10})
        rp:save("test.ohr2.z")
    end)
    it("can be loaded", function()
        ...
        assert.is.equal(rp.data.pack_id, "somepack")
        assert.is.equal(rp.data.level_id, "somelevel")
        assert.is.equal(rp.data.seed, 2323)
    end)
    it("can save and load inputs", function()
        local rp = Replay:new()
        rp:set_game_data({debug = true}, 23453, "somepack", "somelevel", {somesetting = false})
        local to_record = {
            [10] = {
                left = true
            },
        ...
        assert.is.equal(rp.data.seed, 23453)
        assert.is.equal(loaded_rp.data.pack_id, "somepack")
        assert.is.equal(loaded_rp.data.level_id, "somelevel")
