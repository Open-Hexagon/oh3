    ...
    local function ls_crypto_secretbox(basename)
        ...
        return {
            [crypto_secretbox] = function(m, nonce, key)
                ...
                if tonumber(sodium_lib[crypto_secretbox](c, tmp_m, mlen + ZEROBYTES, nonce, key)) == -1 then
            ...
            [crypto_secretbox_open] = function(c, nonce, key)
                ...
                if tonumber(sodium_lib[crypto_secretbox_open](m, tmp_c, clen + BOXZEROBYTES, nonce, key)) == -1 then
    ...
    local function ls_crypto_secretbox_easy(basename)
        ...
        return {
            [crypto_secretbox_easy] = function(m, nonce, key)
                ...
                if tonumber(sodium_lib[crypto_secretbox_easy](c, m, mlen, nonce, key)) == -1 then
            ...
            [crypto_secretbox_open_easy] = function(c, nonce, key)
                ...
                if tonumber(sodium_lib[crypto_secretbox_open_easy](m, c, clen, nonce, key)) == -1 then
            ...
            [crypto_secretbox_detached] = function(message, nonce, key)
                ...
                if tonumber(sodium_lib[crypto_secretbox_detached](c, mac, message, mlen, nonce, key)) == -1 then
            ...
            [crypto_secretbox_open_detached] = function(cipher, mac, nonce, key)
                ...
                if string_len(mac) ~= MACBYTES then
                    return error(string_format("wrong mac size, expected: %d", MACBYTES))
                end
                ...
                if string_len(key) ~= KEYBYTES then
                    return error(string_format("wrong key size, expected: %d", NONCEBYTES))
                end

                local m = char_array(clen)
                if tonumber(sodium_lib[crypto_secretbox_open_detached](m, cipher, mac, clen, nonce, key)) == -1 then
