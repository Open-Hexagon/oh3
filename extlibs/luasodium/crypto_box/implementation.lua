        ...
        return {
            ...
            [crypto_box] = function(m, nonce, pk, sk)
                ...
                if string_len(nonce) ~= NONCEBYTES then
                    return error(string_format("wrong nonce size, expected: %d", NONCEBYTES))
                end

                if string_len(pk) ~= PUBLICKEYBYTES then
                    return error(string_format("wrong key size, expected: %d", PUBLICKEYBYTES))
                end
                ...
                if sodium_lib[crypto_box](c, tmp_m, mlen + ZEROBYTES, nonce, pk, sk) == -1 then
            ...
            [crypto_box_open] = function(c, nonce, pk, sk)
                ...
                if clen < MACBYTES then
                    return error(string_format("wrong c size, expected at least: %d", MACBYTES))
                end
                ...
                if sodium_lib[crypto_box_open](m, tmp_c, clen + BOXZEROBYTES, nonce, pk, sk) == -1 then
            ...
            [crypto_box_beforenm] = function(pk, sk)
                ...
                if string_len(pk) ~= PUBLICKEYBYTES then
                    return error(string_format("wrong public key length, expected: %d", PUBLICKEYBYTES))
                end

                if string_len(sk) ~= SECRETKEYBYTES then
                    return error(string_format("wrong secret key length, expected: %d", SECRETKEYBYTES))
                end
            ...
            [crypto_box_afternm] = function(m, nonce, k)
                ...
                if string_len(k) ~= BEFORENMBYTES then
                    return error(string_format("wrong key size, expected: %d", BEFORENMBYTES))
                end
                ...
                if sodium_lib[crypto_box_afternm](c, tmp_m, mlen + ZEROBYTES, nonce, k) == -1 then
            ...
            [crypto_box_open_afternm] = function(c, nonce, k)
                ...
                if sodium_lib[crypto_box_open_afternm](m, tmp_c, clen + BOXZEROBYTES, nonce, k) == -1 then
        ...
        return {
            [crypto_box_seed_keypair] = function(seed)
                ...
                if string_len(seed) ~= SEEDBYTES then
                    return error(string_format("wrong seed length, expected: %d", SEEDBYTES))
                end
                local pk = char_array(PUBLICKEYBYTES)
                local sk = char_array(SECRETKEYBYTES)
        ...
        return {
            [crypto_box_easy] = function(m, n, pk, sk)
                ...
                if string_len(n) ~= NONCEBYTES then
                    return error(string_format("wrong nonce length, expected: %d", NONCEBYTES))
                end
            ...
            [crypto_box_open_easy] = function(c, n, pk, sk)
                ...
                if clen < MACBYTES then
                    return error(string_format("wrong cipher length, expected at least: %d", MACBYTES))
                end
            ...
            [crypto_box_open_detached] = function(c, mac, n, pk, sk)
                ...
                if maclen ~= MACBYTES then
                    return error(string_format("wrong mac length, expected: %d", MACBYTES))
                end
            ...
            [crypto_box_easy_afternm] = function(m, n, k)
                ...
                if string_len(k) ~= BEFORENMBYTES then
                    return error(string_format("wrong shared key length, expected: %d", BEFORENMBYTES))
                end
