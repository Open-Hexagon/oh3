return function(sodium_lib)
    ...
    local function luasodium_hex2bin(hex, ignore)
        ...
        if sodium_lib.sodium_hex2bin(bin, bin_len, tmp_hex, hex_len, ignore, out_bin_len, hex_end) ~= 0 then
    ...
    local function luasodium_bin2base64(bin, variant)
        ...
        sodium_lib.sodium_bin2base64(b64, b64_len, bin, bin_len, variant)
    ...
    local function luasodium_base642bin(base64, variant, ignore)
        ...
        if
            sodium_lib.sodium_base642bin(bin, bin_len, tmp_base64, base64_len, ignore, out_bin_len, base64_end, variant)
            ~= 0
        then
    ...
    local function luasodium_pad(n, blocksize)
        ...
        if sodium_lib.sodium_pad(outlen, r, nlen, blocksize, rounded) ~= 0 then
    ...
    local function luasodium_unpad(n, blocksize)
        ...
        if sodium_lib.sodium_unpad(outlen, n, nlen, blocksize) ~= 0 then
