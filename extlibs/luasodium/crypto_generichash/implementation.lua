    ...
    local function ls_crypto_generichash(basename)
        ...
        local M = {
            ...
            [crypto_generichash] = function(message, key, outlen)
                ...
                if key then
                    ...
                    if keylen < KEYBYTES_MIN then
                        return error(string_format("key too small, required minimum: %d", KEYBYTES_MIN))
                    elseif keylen > KEYBYTES_MAX then
                        return error(string_format("key too large, required maximum: %d", KEYBYTES_MAX))
                    end
                end

                if outlen then
                    if outlen < BYTES_MIN then
                        return error(string_format("hash too small, required minimum: %d", BYTES_MIN))
                    elseif outlen > BYTES_MAX then
                        return error(string_format("hash too large, required maximum: %d", BYTES_MAX))
                    end
                else
                    outlen = BYTES
                end
                ...
                if
                    tonumber(sodium_lib[crypto_generichash](out, outlen, message, string_len(message), key, keylen))
                    == -1
                then
            ...
            [crypto_generichash_init] = function(key, outlen)
                ...
                local state = ffi.gc(char_array(STATEBYTES), ls_crypto_generichash_free)
            ...
            [crypto_generichash_update] = function(ls_state, message)
                ...
                return tonumber(sodium_lib[crypto_generichash_update](ls_state.state, message, mlen)) == 0
            end,
            ...
            [crypto_generichash_final] = function(ls_state, outlen)
                ...
                if tonumber(sodium_lib[crypto_generichash_final](ls_state.state, out, outlen)) == -1 then
