    ...
    local function ls_crypto_onetimeauth(basename)
        ...
        local M = {
            [crypto_onetimeauth] = function(message, key)
                ...
                if tonumber(sodium_lib[crypto_onetimeauth](auth, message, string_len(message), key)) == -1 then
            ...
            [crypto_onetimeauth_verify] = function(auth, message, key)
                ...
                if string_len(auth) ~= BYTES then
                    return error(string_format("wrong auth size, expected: %d", BYTES))
                end
                ...
                return tonumber(sodium_lib[crypto_onetimeauth_verify](auth, message, string_len(message), key)) ~= -1
            end,
            ...
            [crypto_onetimeauth_init] = function(key)
                ...
                local state = ffi.gc(char_array(STATEBYTES), ls_crypto_onetimeauth_free)
            ...
            [crypto_onetimeauth_update] = function(ls_state, m)
                ...
                return tonumber(sodium_lib[crypto_onetimeauth_update](ls_state.state, m, string_len(m))) ~= -1
            end,
            ...
            [crypto_onetimeauth_final] = function(ls_state)
                ...
                local auth = char_array(BYTES)
                if tonumber(sodium_lib[crypto_onetimeauth_final](ls_state.state, auth)) == -1 then
