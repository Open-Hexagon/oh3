        ...
        return {
            [crypto_sign] = function(m, sk)
                ...
                if string_len(sk) ~= SECRETKEYBYTES then
                    return error(string_format("wrong secret key size, expected: %d", SECRETKEYBYTES))
                end
            ...
            [crypto_sign_open] = function(sm, pk)
                ...
                if string_len(pk) ~= PUBLICKEYBYTES then
                    return error(string_format("wrong public key size, expected: %d", PUBLICKEYBYTES))
                end
    ...
    local function ls_crypto_sign_detached(basename)
        ...
        return {
            ...
            [crypto_sign_verify_detached] = function(sig, m, pk)
                ...
                return tonumber(sodium_lib[crypto_sign_verify_detached](sig, m, string_len(m), pk)) == 0
            end,
        }
    end
    ...
    local function ls_crypto_sign_state(basename, PUBLICKEYBYTES, SECRETKEYBYTES, BYTES)
        ...
        local M = {
            [crypto_sign_init] = function()
                local state = ffi.gc(char_array(STATEBYTES), ls_crypto_sign_free)
            ...
            [crypto_sign_update] = function(ls_state, m)
                ...
                return tonumber(sodium_lib[crypto_sign_update](ls_state.state, m, string_len(m))) ~= -1
            end,
            ...
            [crypto_sign_final_create] = function(ls_state, sk)
                ...
                if tonumber(sodium_lib[crypto_sign_final_create](ls_state.state, sig, siglen, sk)) == -1 then
            ...
            [crypto_sign_final_verify] = function(ls_state, sig, pk)
                ...
                return tonumber(sodium_lib[crypto_sign_final_verify](ls_state.state, sig, pk)) == 0
            end,
        }
    ...
    local function ls_crypto_sign_sk(basename)
        ...
        return {
            [crypto_sign_sk_to_seed] = function(sk)
                ...
                if string_len(sk) ~= SECRETKEYBYTES then
                    return error(string.format("wrong secret key size, expected: %d", SECRETKEYBYTES))
                end

                local seed = char_array(SEEDBYTES)

                if tonumber(sodium_lib[crypto_sign_sk_to_seed](seed, sk)) == -1 then
            ...
            [crypto_sign_sk_to_pk] = function(sk)
                ...
                if tonumber(sodium_lib[crypto_sign_sk_to_pk](pk, sk)) == -1 then
