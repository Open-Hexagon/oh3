        ...
        return {
            [crypto_scalarmult] = function(n, p)
                ...
                if string_len(n) ~= SCALARBYTES then
                    return error(string.format("wrong scalar length, expected: %d", SCALARBYTES))
                end

                if string_len(p) ~= BYTES then
                    return error(string_format("wrong scalar length, expected: %d", BYTES))
                end
            ...
            [crypto_scalarmult_base] = function(n)
                ...
                if string_len(n) ~= SCALARBYTES then
                    return error(string_format("wrong scalar length, expected: %d", SCALARBYTES))
                end
                q = char_array(BYTES)
