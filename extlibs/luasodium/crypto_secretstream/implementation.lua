    ...
    local function ls_crypto_secretstream(basename)
        ...
        local M = {
            ...
            [crypto_secretstream_init_push] = function(key)
                ...
                local state = ffi.gc(char_array(STATEBYTES), ls_crypto_secretstream_free)
            ...
            [crypto_secretstream_push] = function(ls_state, message, tag, ad)
                ...
                if
                    tonumber(
                        sodium_lib[crypto_secretstream_push](ls_state.state, c, clen, message, mlen, ad, adlen, tag)
                    ) == -1
                then
            ...
            [crypto_secretstream_init_pull] = function(header, key)
                ...
                if string_len(header) ~= HEADERBYTES then
                    return error(string_format("wrong header size, expected: %d", HEADERBYTES))
                end
            ...
            [crypto_secretstream_pull] = function(ls_state, cipher, ad)
                ...
                if clen < ABYTES then
                    return error(string_format("invalid cipher length, expected at least: %d", ABYTES))
                end
                ...
                if
                    tonumber(
                        sodium_lib[crypto_secretstream_pull](ls_state.state, m, mlen, tag, cipher, clen, ad, adlen)
                    ) == -1
                then
