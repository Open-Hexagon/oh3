        ...
        return {
            ...
            [crypto_kx_client_session_keys] = function(client_pk, client_sk, server_pk)
                ...
                if string_len(client_pk) ~= PUBLICKEYBYTES then
                    return error("wrong client public key size, expected: %d", PUBLICKEYBYTES)
                end
                if string_len(client_sk) ~= SECRETKEYBYTES then
                    return error("wrong client secret key size, expected: %d", SECRETKEYBYTES)
                end
                if string_len(server_pk) ~= PUBLICKEYBYTES then
                    return error("wrong server public key size, expected: %d", PUBLICKEYBYTES)
                end
                ...
                if
                    tonumber(sodium_lib[crypto_kx_client_session_keys](rx, tx, client_pk, client_sk, server_pk)) == -1
                then
            ...
            [crypto_kx_server_session_keys] = function(server_pk, server_sk, client_pk)
                ...
                if string_len(server_sk) ~= SECRETKEYBYTES then
                    return error("wrong server secret key size, expected: %d", SECRETKEYBYTES)
                end
                if string_len(client_pk) ~= PUBLICKEYBYTES then
                    return error("wrong client public key size, expected: %d", PUBLICKEYBYTES)
                end
                ...
                if
                    tonumber(sodium_lib[crypto_kx_server_session_keys](rx, tx, server_pk, server_sk, client_pk)) == -1
                then
