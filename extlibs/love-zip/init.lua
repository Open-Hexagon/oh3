...
function zapi.createZip(path, zipFile, yield_in_between)
    path = path:gsub("\\", "/")
    ...
    local basename = path:sub(#path:match(".*/") + 1)

    if yield_in_between then
        coroutine.yield()
    end
    ...
    local function index(dir)
        ...
        if dirInfo.type == "file" then
            ...
            local fileName = basename .. "/" .. dir:sub(#path + 2, -1)
...
function zapi.writeZip(path, destination, yield_in_between)
    local zipFile, zipFileErr = love.filesystem.openFile(destination, "w")
    if not zipFile then
        return false, "Failed to open destination zip file: " .. tostring(zipFileErr)
    end

    return zapi.createZip(path, zipFile, yield_in_between)
end
